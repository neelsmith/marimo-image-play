...
def _(mo):
    mo.md("""
    # Show rectangles on image in an HTML canvas
    """)
    return
...
def _(mo, srcw, w):
    mo.md(f"""
    *Width of image to import* {srcw} *Width of display*: {w}
    """)
    return
...
def _(canvas, mo):
    ...
    if isinstance(click, dict) and "x" in click and "y" in click:
        ...
        if all(isinstance(v, int) and v > 0 for v in [disp_w, disp_h, nat_w, nat_h]):
            ...
            x_norm_str = f"{x_norm:.4f}"
            y_norm_str = f"{y_norm:.4f}"
    ...
    **Coordinates of clicked point**  
    ...
    Normalized: x={x_norm_str}, y={y_norm_str}  
    ...
    """.format(
                    ...
                    x_px=x_px,
                    y_px=y_px,
                    x_norm_str=x_norm_str,
                    y_norm_str=y_norm_str,
                    nat_w=nat_w,
                    nat_h=nat_h,
                )
            )
